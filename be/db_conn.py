...
class Order_detail(Base):
    ...
    order_id = Column(String(128), ForeignKey('order.order_id'), nullable=False)
    book_id = Column(String(128), ForeignKey('book.book_id'), primary_key=True, nullable=False)
    count = Column(Integer, nullable=False)


def user_id_exist(user_id):
    cursor = session.query(User).filter(user_id == user_id).first()
    if (cursor is None):
        return False
    else:
        return True
...
def book_id_exist(store_id, book_id):
    cursor = session.query(Store_detail).filter(store_id == store_id, book_id == book_id).first()
    if (cursor is None):
        return False
    else:
        return True
...
def store_id_exist(store_id):
    cursor = session.query(Store).filter(store_id == store_id).first()
    if (cursor is None):
        return False
    else:
        return True